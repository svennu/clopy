# Clopy Removed Documentation

This file preserves the `clopy.md` content that existed before the current implementation-specification rewrite. It was extracted from the diff base (`HEAD~1:clopy.md`) so removed or replaced documentation remains reviewable while `clopy.md` continues to describe the current code.
